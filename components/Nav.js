...
export default function Nav({show, triggerUpdate}) {
    const inactiveLink = 'flex gap-1 p-1 font-semibold';
    const activeLink = inactiveLink+' bg-highlight text-black font-semibold rounded-sm';
    ...
    return (
        <aside className={(show ? 'left-0' : '-left-full')+" top-0 text-gray-500 p-4 fixed w-full bg-backG h-full md:static md:w-auto transition-all"}>
           <div className="flex mb-4 mr-4">
           <Logo/>
           <button onClick={() => triggerUpdate(false)} className={(show ? 'ml-auto relative flex -top-8' : '-left-full')+" flex md:hidden items-center p-4 "}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
            </button>
           </div>
           {/* <button  className="block flex md:hidden items-center p-4"></button> */}
